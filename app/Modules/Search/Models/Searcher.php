...
class Searcher
{
     ...
     * @var Collection
     */
    protected $modelsToSearchThrough;
     ...
     * @var  ?array
     */
    protected $orderByModel = null;

    /**
     * Begin the search term with a wildcard.
     * @var bool
     */
    protected $beginWithWildcard = false;
     ...
     * @var Collection
     */
    protected $termsWithoutWildcards;
     ...
     */
    protected $includeModelTypeWithKey = null;

    /**
     * Include the route name key in the search results.
     */
    protected $includeRouteNameKey = null;

    /**
     * Include the route name value in the search results.
     */
    protected $includeRouteNameValue = null;
    ...
    public function parseTerms(string $terms, callable $callback = null): Collection
    {
        ...
        return Collection::make(str_getcsv($terms, ' ', '"'))
            ...
            ->when($callback !== null, function ($terms) use ($callback) {
                return $terms->each(function ($value, $key) use ($callback) {
                    return $callback ? $callback($value, $key) : null;
                });
     ...
     *
     * @param string $terms
     * @throws \ App\Modules\Search\Models\EmptySearchQueryException
     * @return self
     */
     ...
     * @param \ App\Modules\Search\Models\ModelToSearchThrough $modelToSearchThrough
     * @return void
     */
            ...
            $modelToSearchThrough
                ...
                ->each(function (ModelToSearchThrough $modelToSearchThrough) use ($query) {
                    if ($relation = $modelToSearchThrough->getFullTextRelation()) {
                        ...
                        $query->orWhereFullText(
                            $modelToSearchThrough->getColumns()->map(function ($column) use ($modelToSearchThrough) {
                                return $modelToSearchThrough->qualifyColumn($column);
                            })->all(),
        ...
        $query->orWhereHas($relation, function ($relationQuery) use ($column) {
            $relationQuery->where(
                function ($query) use ($column) {
                    return $this->addWhereTermsToQuery($query, $query->qualifyColumn($column));
                }
     ...
     *
     * @param \ App\Modules\Search\Models\ModelToSearchThrough $currentModel
     * @return array
     */
    ...
    protected function getCompiledQueryBuilder(): QueryBuilder
    {
        ...
        $queries->each(function (Builder $query) use ($firstQuery) {
            $firstQuery->union($query);
        });
    ...
    public function search(string $terms = null)
    {
        ...
        return $results->map(function ($item) use ($modelsPerType) {
            ...
            if ($this->includeRouteNameKey && $this->includeRouteNameValue) {
                $model->setAttribute($this->includeRouteNameKey, $this->includeRouteNameValue);
            }

            return $model;
        })
            ->pipe(function (Collection $models) {
                return new EloquentCollection($models);
            })
            // ->pipe(fn (Collection $models) => new EloquentCollection($models))
            ->when($this->pageName, function (EloquentCollection $models) use ($results) {
                return $results->setCollection($models);
            });
